...
```{r}
Model3<- lm(Balance~Income+Limit+Rating+Cards, data=Credit)
plot(Model3)
summary(Model3)
```
#
Cards is insignificant.  We're going to try and drop that from the model
```{r}
Model4<- lm(Balance~Income+Limit+Rating, data=Credit)
plot(Model4)
summary(Model4)
```
#
Limit becomes insignifacant.  But if your limit is low you can't have a high balance, so shouldn't there be a relationship between balance and limit?
Doing a model of just balance and limit.
```{r}
Model5<- lm(Balance~Income+Limit, data=Credit)
plot(Model5)
summary(Model5)
```
...
```{r}
Model6<- lm(Balance~Income, data=Credit)
plot(Model6)
summary(Model6)
...
```{r}
Model7<-lm(Balance~Income+Rating, data=Credit)
plot(Model7)
summary(Model7)
```

```{r}
Model8<- lm(Balance~Limit+Rating, data=Credit)
plot(Model8)
summary(Model8)
```
